...
mod test {
    ...
    use {BitSliceable, BitVec, Bits, BitsExt, BitsMut};
    ...
    fn assert_0001<T: Bits>(bits: &T) {
        ...
        assert_eq!(bv, bit_vec![T::Block ; false, false, false, true]);
    }

    #[test]
    fn simple_not() {
        let bv: BitVec<usize> = bit_vec![usize; true, true, true, false,];
    ...
    #[test]
    fn simple_and() {
        let bv1: BitVec<u8> = bit_vec![u8; false, false, true, true,];
        let bv2: BitVec<u8> = bit_vec![u8; false, true, false, true,];
    ...
    fn and_with_same_offset() {
        let bv1: BitVec<u8> = bit_vec![u8; true, false, false, true, true];
        let bv2: BitVec<u8> = bit_vec![u8; true, false, true, false, true];
    ...
    fn and_with_different_offset() {
        let bv1: BitVec<u8> = bit_vec![u8; true, true, false, false, true, true];
        let bv2: BitVec<u8> = bit_vec![u8; true, false, true, false, true];
    ...
    #[test]
    fn append() {
        let bv1: BitVec<u8> = bit_vec![u8; false];
        let bv2: BitVec<u8> = bit_vec![u8; true, true];
        let bv3: BitVec<u8> = bit_vec![u8; false, false, false];
    ...
    fn pad() {
        ...
        let bv2: BitVec<usize> = bit_vec![usize; true, true];

        let bv3 = bv1.bit_or(bv2.bit_pad(bv1.bit_len())).to_bit_vec();

        assert_eq!(bv3, bit_vec![usize; true, true, true, false]);
    }

    #[test]
    fn slice_adapter() {
        let bv1: BitVec<usize> =
            bit_vec![usize; true, false, true, false, true, false, true, false];
        let bv2: BitVec<usize> =
            bit_vec![usize; true, true, false, false, true, true, false, false];

        let bv3 = bv1.bit_xor(&bv2).bit_slice(1..7).to_bit_vec();

        assert_eq!(bv3, bit_vec![usize; true, true, false, false, true, true]);
    }

    #[test]
    fn slice_adapter_mutation() {
        let mut bv: BitVec<usize> = bit_vec![usize; true, false, true, false];
        ...
        assert_eq!(bv, bit_vec![usize; true, false, false, false]);
        ...
        assert_eq!(bv, bit_vec![usize; true, true, true, false]);
    }

    #[test]
    fn mixed_equality() {
        let bv1: BitVec<usize> = bit_vec![usize; false, false, true, true];
        let bv2: BitVec<usize> = bit_vec![usize; false, true, false, true];
        assert_eq!(bv1.bit_xor(&bv2), bit_vec![usize; false, true, true, false]);
    }
}
